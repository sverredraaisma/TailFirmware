...
{
    for (int i = 0; i < 2; i++) {
        encoder_init(&halves_[i].encoder, mux, halves_[i].encoder.mux_channel);
    }
}
...
void AxisController::update(i2c_mux_t *mux, float dt)
{
    for (int i = 0; i < 2; i++) {
        ...
        /* Read encoder and compute position relative to zero */
        ...
        /* Apply inversion if servo spins opposite to encoder direction */
        if (h.invert) {
            output = -output;
        }

        /* Convert PID output (float) to servo speed (int16_t) and drive servo */
